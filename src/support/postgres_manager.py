...
def list_available_backup():
    ...
    s3_objects = s3_client.list_objects_v2(
        Bucket=AWS_BUCKET_NAME, Prefix=AWS_BUCKET_PATH
    )
...
# --dbname=postgresql://postgres:Syjrl7heXR@192.168.64.2:30501
...
def remove_faulty_statement_from_dump(src_file):
    ...
    try:
        with open(temp_file, "w+") as dump_temp:
            process = subprocess.Popen(
                ["pg_restore", "-l" "-v", src_file], stdout=subprocess.PIPE
            )
...
def create_db(db_host, database, db_port, user_name, user_password):
    ...
    cur.execute(
        "GRANT ALL PRIVILEGES ON DATABASE {} TO {} ;".format(database, user_name)
    )
    return database


def swap_restore_active(
    db_host, restore_database, active_database, db_port, user_name, user_password
):
    try:
        con = psycopg2.connect(
            dbname="postgres",
        ...
        cur.execute(
            'ALTER DATABASE "{}" RENAME TO "{}";'.format(
                restore_database, active_database
            )
        )
    except Exception as e:
        print(e)
...
def swap_restore_new(
    db_host, restore_database, new_database, db_port, user_name, user_password
):
    try:
        con = psycopg2.connect(
            dbname="postgres",
        ...
        cur = con.cursor()
        cur.execute(
            'ALTER DATABASE "{}" RENAME TO "{}";'.format(restore_database, new_database)
        )
    except Exception as e:
        print(e)
        exit(1)
...
def main():
    ...
    if args.action == "list":
        logger.info(
            "Listing S3 bucket s3://{}/{} content :".format(
                AWS_BUCKET_NAME, AWS_BUCKET_PATH
            )
        )
    ...
    # restore task
        ...
        if not args.date:
            ...
            if backup_match:
                ...
                logger.error(
                    "No match found for backups with date : {}".format(args.date)
                )
                logger.info("Available keys : {}".format([s for s in all_backup_keys]))
                exit(1)

            logger.info(
                "Downloading {} from S3 into : {}".format(
