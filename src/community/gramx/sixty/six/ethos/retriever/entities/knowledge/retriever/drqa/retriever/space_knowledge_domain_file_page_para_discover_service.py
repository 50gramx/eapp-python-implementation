...
class SpaceKnowledgeDomainFilePageParaDB(object):
    ...
    def __init__(self, space_knowledge_domain_id: str, db_path=None):
        ...
        self.para_table_name = f"skdfppt_{space_knowledge_domain_id}"
    ...
    def get_doc_ids(self):
        ...
        cursor.execute(f'SELECT para_id FROM "{self.para_table_name}"')
        results = [r[0] for r in cursor.fetchall()]
        cursor.close()
        return results

    def get_doc_text(self, para_id):
        """Fetch the raw text of the doc for 'doc_id'."""
        cursor = self.connection.cursor()
        cursor.execute(
            f'SELECT para_text FROM "{self.para_table_name}" WHERE para_id = %s',
            (para_id,)
        )
