...
class KnowledgeSpace:
    def __init__(self, space_knowledge_id: str):
        ...
        try:
            self.domain_table = KnowledgeSpaceModels.metadata.tables[self.domain_model_name]
            self.inferred_domain_table = KnowledgeSpaceModels.metadata.tables[self.inferred_domain_model_name]
        except KeyError:
    ...
    def get_domain_model(self):
        class SpaceKnowledgeDomain(KnowledgeSpaceModels):
            ...
            space_knowledge_domain_id = Column(String(255), primary_key=True, unique=True)
    ...
    def add_new_domain(self, domain_name: str, domain_description: str, domain_collar_enum: int,
        ...
        domain_id = gen_uuid()
        statement = KnowledgeSpaceModels.metadata.tables[self.domain_model_name].insert().values(
        ...
        with DbSession.session_scope() as session:
            ...
            session.commit()
        domain_knowledge_space = DomainKnowledgeSpace(space_knowledge_id=self.space_knowledge_id,
                                                      space_knowledge_domain_id=domain_id)
    ...
    def get_domain_with_id(
            self,
            space_knowledge: space_knowledge_pb2.SpaceKnowledge,
            domain_id: str
    ) -> space_knowledge_domain_pb2.SpaceKnowledgeDomain:
        with DbSession.session_scope() as session:
            if domain_id != "":
                space_knowledge_domain = session.query(self.domain_table).filter(
                    self.domain_table.c.space_knowledge_domain_id == domain_id
                ).first()
            else:
                space_knowledge_domain = session.query(self.domain_table).filter(
                    self.domain_table.c.space_knowledge_domain_collar_enum == 0
                ).first()
            if space_knowledge_domain is None:
                ...
                return space_knowledge_domain_pb2.SpaceKnowledgeDomain(
                    ...
                    created_at=format_datetime_to_timestamp(space_knowledge_domain.created_at),
                    last_updated_at=format_datetime_to_timestamp(space_knowledge_domain.last_updated_at)
                )
    ...
    def add_new_inferred_domain(self, inferred_domain_id: str, space_knowledge_id: str, space_knowledge_domain_id: str):
        statement = KnowledgeSpaceModels.metadata.tables[self.inferred_domain_model_name].insert().values(
    ...
    def is_existing_inferred_domain(self, inferred_domain_id: str, space_knowledge_id: str,
                                    space_knowledge_domain_id: str) -> bool:
        with DbSession.session_scope() as session:
            statement = session.query(self.inferred_domain_table).filter(
                ...
                self.inferred_domain_table.c.space_knowledge_domain_id == space_knowledge_domain_id
            )
    ...
    def get_inferred_domain_all(self):
        with DbSession.session_scope() as session:
            all_space_knowledge_domain_inferred = session.query(self.inferred_domain_table).all()
            return [space_knowledge_domain_pb2.SpaceKnowledgeDomainInferred(
                ...
                inferred_at=format_datetime_to_timestamp(space_knowledge_domain_inferred.inferred_at)
