...
class DiscoverSpaceKnowledgeDomainConsumer:
    ...
    @staticmethod
    def get_best_answers(access_auth_details: SpaceKnowledgeDomainServicesAccessAuthDetails,
                         best_answers_count: int,
                         question: str) -> (bool, str, [RankedAnswer]):
        stub = ApplicationContext.discover_space_knowledge_domain_service_stub()
        request = GetBestAnswersRequest(
            access_auth_details=access_auth_details, best_answers_count=best_answers_count, question=question)
        response = stub.GetBestAnswers(request)
        return response.response_meta.meta_done, response.response_meta.meta_message, response.ranked_answers
