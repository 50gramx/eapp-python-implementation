...
from ethos.elint.services.product.knowledge.space_knowledge_domain.create_space_knowledge_domain_pb2 import (
    CreateAccountWhiteSpaceKnowledgeDomainResponse,
...
from community.gramx.fifty.zero.ethos.knowledge_spaces.models.knowledge_space_models import (
...
class CreateSpaceKnowledgeDomainService(CreateSpaceKnowledgeDomainServiceServicer):
    ...
    def CreateAccountWhiteSpaceKnowledgeDomain(self, request, context):
        logging.info(
            "CreateSpaceKnowledgeDomainService:CreateAccountWhiteSpaceKnowledgeDomain invoked."
        )
        space_knowledge = request.space_knowledge7
        validation_done, validation_message = (
            AccessSpaceKnowledgeConsumer.validate_space_knowledge_services(request)
        )
        if validation_done is False:
            ...
            space_knowledge_domain_id = knowledge_space.add_new_domain(
                domain_name="My White Knowledge Domain",
                domain_description="My Information",
                domain_collar_enum=0,
                domain_isolate=True,
            )
            # TODO: fix this
            knowledge_space.add_new_inferred_domain(
                space_knowledge_id=space_knowledge.space_knowledge_id,
            ...
            # TODO: fix this
            domain_knowledge_space.add_new_inferring_account(
                space_knowledge_id=space_knowledge.space_knowledge_id,
                account_id=space_knowledge.space.space_admin_id,
            )
            access_auth_details, access_done, access_message = (
    ...
    def CreateSpaceKnowledgeDomain(self, request, context):
        logging.info(
            "CreateSpaceKnowledgeDomainService:CreateSpaceKnowledgeDomain invoked."
        )
        validation_done, validation_message = (
            AccessSpaceKnowledgeConsumer.validate_space_knowledge_services(
        ...
        if validation_done is False:
            ...
            if charge_for_closed_domain_launch_response.meta_done is False:
                ...
                space_knowledge = (
                    request.space_knowledge_services_access_auth_details.space_knowledge
                )
                knowledge_space = KnowledgeSpace(
                    space_knowledge_id=space_knowledge.space_knowledge_id
                )
