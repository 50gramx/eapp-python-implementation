...
from ethos.elint.services.product.conversation.message.account.receive_account_message_pb2 import (
    ...
    AccountAssistantReceivedMessagesCountResponse,
    AccountReceivedMessage,
    AccountReceivedMessagesCountResponse,
    GetAccountAssistantLastReceivedMessageResponse,
    GetAccountLastReceivedMessageResponse,
    GetLast24ProductNReceivedMessagesResponse,
    GetReceivedMessagesAccountAssistantsResponse,
    GetReceivedMessagesAccountsResponse,
    ListenForReceivedAccountAssistantMessagesResponse,
    ListenForReceivedAccountMessagesResponse,
    ListenForReceivedAccountSpeedMessagesResponse,
    MessageFromAccount,
    MessageFromAccountAssistant,
    MessageFromAccountAssistantReceived,
    ...
    SyncAccountReceivedMessagesResponse,
)
...
from ethos.elint.services.product.identity.account.discover_account_pb2 import (
...
from community.gramx.fifty.zero.ethos.conversations.models.account_conversation_models import (
    AccountConversations,
)
from community.gramx.fifty.zero.ethos.conversations.models.base_models import (
    AccountSpeedMessaging,
)
from community.gramx.fifty.zero.ethos.conversations.services_caller.account_service_caller import (
    is_account_assistant_connected_caller,
    is_account_connected_caller,
    new_received_message_from_account_assistant_caller,
    new_received_message_from_account_caller,
...
from support.session.redis_service import (
    get_kv,
    quick_store_list_length,
    quick_store_list_pop_all_items,
    rpush,
    set_kv,
)
...
class ReceiveAccountMessageService(ReceiveAccountMessageServiceServicer):
    ...
    def ReceiveMessageFromAccountAssistant(self, request, context):
        ...
        if is_connected is False:
            ...
            rpush(
                list_key=f"raam_{request.account_id}",
            ...
            return MessageFromAccountAssistantReceived(
                is_received=True, received_at=received_at
            )
    ...
    def ReceiveMessageFromAccount(self, request, context):
        ...
        is_connected, connection_message = is_account_connected_caller(
            account_id=request.account_id, connected_account=request.connected_account
        )
        if is_connected is False:
            ...
            rpush(
                list_key=f"rm_{request.account_id}",
    ...
    def SyncAccountReceivedMessages(self, request, context):
        ...
        if validation_done is False:
            ...
            account_conversations = AccountConversations(
                account_id=request.account.account_id
            )
            for (
                account_received_message
            ) in account_conversations.get_all_account_received_messages():
    ...
    def SyncAccountConnectedAccountReceivedMessages(self, request, context):
        ...
        if validation_done is False:
            return SyncAccountConnectedAccountReceivedMessagesResponse(
                response_meta=response_meta
            )
        else:
            account_conversations = AccountConversations(
            ...
            for (
                account_received_message_dict
            ) in account_conversations.get_all_account_connected_account_received_messages(
                account_id=request.connected_account.account_id
            ):
                yield SyncAccountConnectedAccountReceivedMessagesResponse(
                    account_received_message=account_received_message_dict.get(
    ...
    def SyncAccountConnectedAccountAssistantReceivedMessages(self, request, context):
        logging.info(
            "ReceiveAccountMessageService:SyncAccountConnectedAccountAssistantReceivedMessages"
        )
        validation_done, validation_message = validate_account_services_caller(
            request.access_auth_details
        )
        ...
        if validation_done is False:
            return SyncAccountConnectedAccountAssistantReceivedMessagesResponse(
                response_meta=response_meta
            )
        else:
            account_conversations = AccountConversations(
            ...
            for (
                account_assistant_received_message_dict
            ) in account_conversations.get_all_account_connected_account_assistant_received_messages(
                account_assistant_id=request.connected_account_assistant.account_assistant_id
            ):
                yield SyncAccountConnectedAccountAssistantReceivedMessagesResponse(
                    ...
                    response_meta=response_meta,
                    sync_progress=account_assistant_received_message_dict.get(
                        "progress", 0.0
                    ),
                )
    ...
    def ListenForReceivedAccountAssistantMessages(self, request, context):
        logging.info(
            "ReceiveAccountMessageService:ListenForReceivedAccountAssistantMessages"
        )
        validation_done, validation_message = validate_account_services_caller(
            request.access_auth_details
        )
        ...
        if validation_done is False:
            ...
            received_message_from_account_assistant_list = (
                quick_store_list_pop_all_items(
    ...
    def ListenForReceivedAccountMessages(self, request, context):
        ...
        if validation_done is False:
            ...
            for message in received_message_from_account_list:
                list_of_received_messages.append(
                    Parse(text=message, message=MessageFromAccount())
                )
    ...
    def ListenForReceivedAccountSpeedMessages(self, request, context):
        logging.info(
            "ReceiveAccountMessageService:ListenForReceivedAccountSpeedMessages"
        )
        validation_done, validation_message = validate_account_services_caller(request)
        response_meta = ResponseMeta(
            meta_done=validation_done, meta_message=validation_message
        ...
        if validation_done is False:
            return ListenForReceivedAccountSpeedMessagesResponse(
                response_meta=response_meta
            )
        else:
            account_id = request.account.account_id
            ...
            context.add_callback(
                lambda: account_speed_messaging.account_speed_message_listening_off()
            )
            while True:
                ...
                if PENDING_MESSAGE is not None and int(PENDING_MESSAGE) == 1:
                    ...
                    for message in received_message_from_account_list:
                        yield ListenForReceivedAccountSpeedMessagesResponse(
                            messages_from_account=Parse(
                                text=message, message=MessageFromAccount()
                            ),
                            response_meta=response_meta,
                        )
                    # check if there is no more pending messages before updating the pending status
                    if quick_store_list_length(list_key=f"rm_{account_id}") == 0:
    ...
    def GetLast24ProductNReceivedMessages(self, request, context):
        ...
        if validation_done is False:
            ...
            time_24_hours_product_n_ago = datetime.datetime.now() - datetime.timedelta(
                days=request.product_n
            )
            if request.message_entity_enum == 0:
                ...
                for (
                    ...
                    account_received_messages.append(
                        account_received_message_dict.get(
                            "message", AccountReceivedMessage()
                        )
                    )
                for (
                    ...
                    account_assistant_received_messages.append(
                        account_assistant_received_message_dict.get(
                            "message", AccountAssistantReceivedMessage()
                        )
                    )
                return GetLast24ProductNReceivedMessagesResponse(
    ...
    def GetAccountAssistantReceivedMessagesCount(self, request, context):
        logging.info(
            "SendAccountMessageService:GetAccountAssistantReceivedMessagesCount"
        )
        validation_done, validation_message = validate_account_services_caller(request)
        response_meta = ResponseMeta(
            meta_done=validation_done, meta_message=validation_message
    ...
    def GetAccountLastReceivedMessage(self, request, context):
        ...
        if validation_done is False:
            ...
            last_received_message = (
                account_conversations.get_account_last_received_message(
                    account_id=request.connected_account_id
                )
            )
            return GetAccountLastReceivedMessageResponse(
                response_meta=response_meta, last_received_message=last_received_message
            )
    ...
    def GetAccountAssistantLastReceivedMessage(self, request, context):
        ...
        if validation_done is False:
            return GetAccountAssistantLastReceivedMessageResponse(
                response_meta=response_meta
            )
        else:
            account_conversations = AccountConversations(
            ...
            last_received_message = (
                account_conversations.get_account_assistant_last_received_message(
    ...
    def GetReceivedMessagesAccounts(self, request, context):
        ...
        if validation_done is False:
            ...
            received_messages_account_ids = (
                account_conversations.get_received_messages_account_ids()
            )
            ...
            for received_messages_account_id in received_messages_account_ids:
                received_messages_accounts.append(
                    ApplicationContext.discover_account_service_stub()
                    .GetAccountById(
                        GetAccountByIdRequest(account_id=received_messages_account_id)
                    )
                    .account
                )
    ...
    def GetReceivedMessagesAccountAssistants(self, request, context):
        ...
        if validation_done is False:
            ...
            received_messages_account_assistant_ids = (
                account_conversations.get_received_messages_account_assistant_ids()
            )
            received_messages_account_assistants = []
            for (
                received_messages_account_assistant_id
            ) in received_messages_account_assistant_ids:
                received_messages_account_assistants.append(
                    ApplicationContext.discover_account_assistant_service_stub()
